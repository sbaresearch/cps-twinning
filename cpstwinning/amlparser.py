...
class AmlParser(object):
    ...
    def run(self):
        ...
        def get_plc_modbus_map(el):
            ...
            def get_plc_mb_map_links(el):
                links = []
                for link in el.xpath('./InternalElement[@RefBaseSystemUnitPath=\"LogicalDeviceSystemUnitClassLib/IOController\"]/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ModbusRoleClassLib/ModbusMap\"]/..//InternalElement/InternalLink'):
                    links.append({'name': get_name_attr(link), 'a': link.attrib['RefPartnerSideA'], 'b': link.attrib['RefPartnerSideB']})
                return links
            ...
            def get_plc_var_name_by_mb_ref(links, ref):
                ...
                value = el.xpath('./InternalElement[@ID=\"{}\"]/ExternalInterface[@Name=\"{}\" and @RefBaseClassPath=\"AutomationMLInterfaceClassLibLogics/VariableInterface\"]/Attribute[@Name=\"refURI\"]/Value/text()'.format(partner_ref_id, partner_ref_name))
                if len(value):
            ...
            def get_mapping(table, links, table_name):
                res = {}
                tbl = table.xpath('./InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ModbusRoleClassLib/ModbusSlave/Tables/{}\"]/..'.format(table_name))
                if len(tbl):
                    id = get_id_attr(tbl[0])
                    for ch in tbl[0].iter("ExternalInterface"):
                        name = get_name_attr(ch)
                        mb_ref = '{{{}}}:{}'.format(id, name)
                        plc_var_name = get_plc_var_name_by_mb_ref(links, mb_ref)
                        res[int(name)] = plc_var_name
                return res
            
            mb_links = get_plc_mb_map_links(el)
            mb_tables = el.xpath('./InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ProgramBlocksRoleClassLib/Main\"]/../InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ModbusRoleClassLib/ModbusTCPSlave\"]/../InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ModbusRoleClassLib/ModbusSlave/Tables\"]/..')
            if len(mb_tables):
        ...
        def get_plc_st_path(el):
            l = el.xpath('./InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ProgramBlocksRoleClassLib/Main\"]/../ExternalInterface[@RefBaseClassPath=\"AutomationMLInterfaceClassLib/AutomationMLBaseInterface/ExternalDataConnector/PLCopenXMLInterface\"]/Attribute[@Name=\"refSTURI\"]/Value/text()')
            return l[0] if len(l) else None
        ...
        def get_network_config(el):
            ...
            for a in el.xpath('./InternalElement/ExternalInterface[@RefBaseClassPath=\"CommunicationInterfaceClassLib/CommunicationPhysicalSocket\"]//Attribute'):
        ...
        def get_plc_var_map(el, motor_vars):
            
            def get_plc_var_map_links(el):
                links = []
                for link in el.xpath('./InternalElement[@RefBaseSystemUnitPath=\"CPSTwinningSystemUnitClassLib/PLCVarMap\"]//InternalLink'):
                    links.append({'name': get_name_attr(link), 'a': link.attrib['RefPartnerSideA'], 'b': link.attrib['RefPartnerSideB']})
                return links
            ...
            for link in plc_var_links:
                ...
                for _ in range(2):
                    ...
                    ext_intf = root.xpath('.//InternalElement[@ID=\"{}\"]/ExternalInterface[@Name=\"{}\"]'.format(ref_partner_id, ref_partner_name))
                    if len(ext_intf):
                        ...
                        if base_class_path == 'AutomationMLInterfaceClassLibLogics/VariableInterface':
                            ...
                            if base_system_unit_path == 'CPSTwinningSystemUnitClassLib/PLCVarMap':
                                ...
                                motor_var_idx = motor_vars.index(filter(lambda n: n.get('name') == motor_var_name, motor_vars)[0])
                    side = 'b'
        ...
        def get_motor_vars(el):
            vars = []
            vars_ext_intf = el.xpath('./InternalElement[@RefBaseSystemUnitPath=\"CPSTwinningSystemUnitClassLib/PLCVarMap\"]//ExternalInterface') 
            for v in vars_ext_intf:
                name = get_name_attr(v)
                type = v.xpath('./Attribute[@Name=\"type\"]/Value/text()')
                if len(type):
                    initVal = None
                    t = type[0]
                    if t == 'int':
                    ...
                    vars.append({'name': name, 'value': initVal})
                    
            return vars
        
        def get_plc_name_for_motor(el):
            
            def get_physical_network_partner_ref(motor_control_ref):
                link = root.xpath('./InstanceHierarchy/InternalElement/' + 
                                  'RoleRequirements[@RefBaseRoleClassPath=\"CommunicationRoleClassLib/PhysicalNetwork\"]/' + 
                                  '../InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"CommunicationRoleClassLib/PhysicalNetwork/PhysicalConnection\"]/' + 
                                  '../InternalLink[@RefPartnerSideA=\"{}\" or @RefPartnerSideB=\"{}\"]'\
                                  .format(motor_control_ref, motor_control_ref))
                if len(link):
                    li = link[0]
            ...
            def get_name_by_ref(ref):
                id = ref[ref.find("{") + 1:ref.find("}")]
                ie_plc = root.xpath('./InstanceHierarchy/InternalElement/' + 
                                    'RoleRequirements[@RefBaseRoleClassPath=\"AutomationMLCSRoleClassLib/ControlEquipment/Controller/PLC\"]/' + 
                                    '../InternalElement[@ID=\"{}\"]/..'.format(id))
                if len(ie_plc):
                    return get_name_attr(ie_plc[0])
                
            # First get MotorControl ref
            ie_motor_control = el.xpath('./InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ConveyorComponentsRoleClassLib/Motor/MotorControl\"]/..')
        ...
        def get_switch_links(el):
            
            def get_switch_endpoints_refs(el):
                endpoints = el.xpath('./InternalElement/ExternalInterface[@RefBaseClassPath=\"CommunicationInterfaceClassLib/SwitchCommunicationPhysicalSocket\"]')
                if len(endpoints):
                    e0 = endpoints[0]
            ...
            def get_name_by_ref(ref):
                id = ref[ref.find("{") + 1:ref.find("}")]
                ie = root.xpath('./InstanceHierarchy/InternalElement/InternalElement[@ID=\"{}\"]/..'.format(id))
                if len(ie):
                    return get_name_attr(ie[0])
                
            endpoint_refs = get_switch_endpoints_refs(ie_switch)
            internal_links = root.xpath('./InstanceHierarchy/InternalElement' + 
                                         '/RoleRequirements[@RefBaseRoleClassPath=\"CommunicationRoleClassLib/PhysicalNetwork\"]' + 
                                        '/../InternalElement/InternalLink')
            partner_refs = []
            sideA = 'RefPartnerSideA'
            sideB = 'RefPartnerSideB'
            for i_link in internal_links:
                if i_link.get(sideA) in endpoint_refs:
                    partner_refs.append(i_link.get(sideB))
                elif i_link.get(sideB) in endpoint_refs:
                    partner_refs.append(i_link.get(sideA))
        ...
        def retrieve_var_constraints(el, plc_name):
            attrs_w_constraints = el.xpath('./InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ProgramBlocksRoleClassLib/Main\"]' + 
        ...
        def get_hmi_vars(el):
            ...
            def get_plc_var_partner_ref(hmi_var_ref):
                i_links = root.xpath('./InstanceHierarchy/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"CommunicationRoleClassLib/LogicalNetwork\"]' + 
                           '/../InternalElement[@RefBaseSystemUnitPath=\"LogicalConnectionSystemUnitClassLib/LogicalConnection\"]' + 
                           '/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ModbusRoleClassLib/ModbusTCPDataPackage\"]' + 
                           '/../InternalLink[@RefPartnerSideA=\"{}\" or @RefPartnerSideB=\"{}\"]'\
            ...
            ext_intf_hmi_vars = el.xpath('./InternalElement[@RefBaseSystemUnitPath=\"LogicalDeviceSystemUnitClassLib/HMI\"]' + 
        ...
        for ie_hmi in root.xpath('./InstanceHierarchy/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"AutomationMLExtendedRoleClassLib/HMI\"]/..'):
            hmi = {}
            hmi['name'] = get_name_attr(ie_hmi)
            hmi['network'] = get_network_config(ie_hmi)
            # get_hmi_vars(ie_hmi)
            self.hmis.append(hmi)
        
        for ie_plc in root.xpath('./InstanceHierarchy/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"AutomationMLCSRoleClassLib/ControlEquipment/Controller/PLC\"]/..'):
            plc = {}
            plc['name'] = get_name_attr(ie_plc)
            plc['network'] = get_network_config(ie_plc)
        ...
        for ie_motor in root.xpath('./InstanceHierarchy/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ConveyorComponentsRoleClassLib/Motor\"]/..'):
            motor = {}
            motor['name'] = get_name_attr(ie_motor)
            motor['vars'] = get_motor_vars(ie_motor)
            motor['plc_var_map'] = get_plc_var_map(ie_motor, motor['vars'])
            motor['plc_name'] = get_plc_name_for_motor(ie_motor)
            self.motors.append(motor)
        
        for ie_switch in root.xpath('./InstanceHierarchy/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"NetworkComponentsRoleClassLib/Switch\"]/..'):
            switch = {}
            switch['name'] = get_name_attr(ie_switch)
            switch['links'] = get_switch_links(ie_switch)
        ...
        for ie_var_link_constraints in root.xpath('./InstanceHierarchy/InternalElement/RoleRequirements[@RefBaseRoleClassPath=\"ConstraintRoleClassLib/VariableLinkConstraintRoleClass\"]/..'):
            constraint = {}
...
if __name__ == '__main__':
    AmlParser('/home/meckhart/Development/AML/cpstwinning/ConveyorSystem.aml')
